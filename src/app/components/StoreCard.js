...
export function StoreCard({ store }) {
  ...
  return (
    <Link href={store.link ? store.link : `/more/${store.id}`}>
      <div className="w-full h-[40vh] border border-gray-200 rounded-2xl p-5 bg-[#F1EFEA] flex-shrink-0 flex flex-col justify-between items-center m-2 shadow-lg transition-transform duration-300 transform hover:scale-105">
        <div className="flex flex-col h-full w-full">
          <div className="flex justify-center items-center mb-4">
            <h2 className="text-gray-800/70 text-2xl font-semibold text-center">
              {store.name}
            </h2>
          </div>
          {store.image && (
  ...
  const [emblaRef, emblaApi] = useEmblaCarousel({
    ...
    breakpoints: {
      ...
      "(min-width: 768px) and (max-width: 1023px)": {
        slidesToScroll: 2,
        align: "start",
      }, // Medium screens: 2 cards visible, scroll 2
        ...
        <div className="embla__container flex">
          ...
          {data.map((store, index) => (
            <div
              ...
              style={{
                minWidth: `calc(100% / var(--slides-per-view, 4))`,
                paddingLeft: "1rem",
                paddingRight: "1rem",
              }} // Apply minWidth and padding for consistent sizing and gap
            >
