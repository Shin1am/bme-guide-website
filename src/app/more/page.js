...
export default function More() {
  ...
  const filteredClub = (() => {
    ...
    const getSearchMatch = (
      c // Helper function for search query
    ) =>
      searchQuery === "" ||
  ...
  return (
    <div className="p-4 min-h-screen">
          ...
          >
            <div className="flex flex-col md:flex-row gap-10 justify-start items-center">
              <div className="flex w-full h-[10vh] md:w-1/2 md:h-[20vh] relative">
                <Image
                  src={club.picture}
                  alt={`${club.title} picture`}
                  fill
                  className="object-cover rounded-xl border-2 shadow-lg hover:scale-103"
                />
              </div>
              <div className="flex flex-row md:flex-col border-2 w-full h-full md:h-[20vh]">
                <div className="flex justify-start md:border-b-2">
                  <h3 className="text-xl md:text-3xl lg:text-2xl xl:text-3xl py-3 px-4">
                    {club.title}
                  </h3>
                </div>
                <div className="md:mt-6 w-full">
                  <p className="text-xs md:text-base lg:text-xs xl:text-base py-2 md:py-0 px-4">
                    {club.description}
                  </p>
                </div>
              </div>
      ...
      <div className="mt-20 lg:ml-30 flex flex-col justify-center lg:justify-start">
        ...
        <a
          href="https://mustudent.mahidol.ac.th/2022/07/27973/"
          target="_blank"
          rel="noopener noreferrer"
          className="text-xl text-blue-500 hover:underline mt-10 mb-10 text-center md:text-left"
        >
          List of food, cafe shop in mahidol
        </a>
      </div>
